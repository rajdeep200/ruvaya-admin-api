...
export function handleError(error:unknown){ if(error instanceof ApiError)return fail(error.code,error.message,error.status,error.details); console.error(error); return fail("INTERNAL_ERROR","An unexpected error occurred. Retry is safe for read operations.",500); }
